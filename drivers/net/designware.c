...
static int dw_eth_init(struct eth_device *dev, bd_t *bis)
{
	...
	struct eth_dma_regs *dma_p = priv->dma_regs_p;
	u32 conf;
    printf("dw_eth_init begin, priv->phy_configured is %d\n", priv->phy_configured);
	if (priv->phy_configured != 1) {
...
#if 0 //defined(CONFIG_DW_AUTONEG)
        int index = 0;
        short value = 0xffff;
        ...
        {
            value = 0xffff;
            eth_mdio_read(dev, phy_addr, index, &value);
...
static int dw_reset_phy(struct eth_device *dev)
{
    ...
    uchar chip = 0x21;
    uint  addr = 0;
    int   alen = 1;       /* dev kit using 2 byte addressing */
    int   linebytes = 2;
    u16   value = 0;
    ...
    {
        value = 0x1234;
        i2c_write(chip, addr, alen, &value, linebytes);
    }
    
    for(addr = 0; addr < 8; addr++)
    {
        value = 0x80;
        i2c_read(chip, addr, alen, &value, linebytes);
        printf("PCA9535 addr %d is 0x%04x \n", addr, value);
    }
#endif
    ...
    eth_mdio_write(dev, phy_addr, 0xC00, BMCR_RESET);
	start = get_timer(0);
	while (get_timer(start) < timeout) {
        eth_mdio_read(dev, phy_addr, 0xC00, &ctrl);
		if (!(ctrl & BMCR_RESET))
			break;
	...
	if (get_timer(start) >= CONFIG_PHYRESET_TIMEOUT) {
		printf("timed out waiting for fiber reset %x\n", ctrl);
		return -1;
	}
#endif
...
static int configure_phy(struct eth_device *dev)
{
	...
	if (phy_addr >= 0){   
        printf("configure_phy phy_addr is 0x%x\n", phy_addr);
	 ...
	 * is run.
	 */
	if (designware_board_phy_init(dev, phy_addr, eth_mdio_write, dw_reset_phy) < 0)
    {
        return -1;
	}
...
#if (CONFIG_EMAC_BASE == CONFIG_EMAC1_BASE)
	eth_mdio_write(dev, phy_addr, MII_ADVERTISE, 0x1E1);
    eth_mdio_read(dev, phy_addr, MII_BMCR, &bmcr);
#endif
#if (CONFIG_EMAC_BASE == CONFIG_EMAC0_BASE)
    eth_mdio_read(dev, phy_addr,  0xC00, &bmcr);
#endif
...
#endif
#if (CONFIG_EMAC_BASE == CONFIG_EMAC0_BASE)
	if (eth_mdio_write(dev, phy_addr, 0xC00, bmcr) < 0)
#endif
		return -1;
...
#endif
#if (CONFIG_EMAC_BASE == CONFIG_EMAC0_BASE)
        eth_mdio_read(dev, phy_addr, 0xC01, &bmsr);
#endif
...
int designware_initialize(u32 id, ulong base_addr, u32 phy_addr, u32 interface)
{
    ...
    value = 0;
    i2c_write(chip, 4, 1, &value, 1);
    i2c_write(chip, 5, 1, &value, 1);

    value = 0xFF;
    i2c_write(chip, 6, 1, &value, 1);
    value = 0x48;
    i2c_write(chip, 7, 1, &value, 1);
    
    value = 0x37;
    i2c_write(chip, 3, 1, &value, 1);
    for(addr = 0; addr < 8; addr++)
    {
        uchar val = 0x80;
        i2c_read(chip, addr, 1, &val, 1);
        printf("PCA9535 addr %d is 0x%02x \n", addr, val);
    }
#endif
	...
	priv->dma_regs_p = (struct eth_dma_regs *)(base_addr + DW_DMA_BASE_OFFSET);
	priv->address = phy_addr;
