...
static void setenv_ethaddr_eeprom(void)
{
	...
	addr = 0x174;
#endif
	//uchar setaddr[6] = {0x12, 0xed, 0xc3, 0x43, 0x32,0x12};
	//i2c_write(chip, addr, alen, setaddr, linebytes);
		...
		int (*dw_reset_phy)(struct eth_device *))
{
	...
	struct mii_dev *bus;
    printf("designware_board_phy_init begin \n");
	if ((*dw_reset_phy)(dev) < 0)
		return -1;
    printf("designware_board_phy_init 111 \n");
	bus = mdio_get_current_dev();
	phydev = phy_connect(bus, phy_addr, dev, priv->interface);
    printf("designware_board_phy_init 222 phydev point ix 0x%p\n", phydev);
    printf("phydev->phy_id is 0x%x\n", phydev->phy_id);
#if 1
	if (strcasecmp(phydev->drv->name, "TI dp83869hm") == 0 &&
...
#endif
    printf("designware_board_phy_init end \n");
	return 0;
}
#endif
