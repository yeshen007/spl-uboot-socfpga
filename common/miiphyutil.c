...
int miiphy_speed(const char *devname, unsigned char addr)
{
    ...
    {   
        printf("miiphy_speed miiphy_is_1000base_x\n");
		return _1000BASET;
	}
    ...
    {  
        printf("miiphy_speed _1000BASET, btsr ix 0x%x\n", btsr);
		return _1000BASET;
 	}
...
int miiphy_duplex(const char *devname, unsigned char addr)
{
	...
	if (miiphy_is_1000base_x(devname, addr)) {
		/* 1000BASE-X */
		if (miiphy_read(devname, addr, 0xC05, &anlpar)) {
			printf("1000BASE-X PHY AN duplex");
			goto miiphy_read_failed;
		}

        return (anlpar & LPA_1000XFULL) ? FULL : HALF;
	} 
	/*
	 * No 1000BASE-X, so assume 1000BASE-T/100BASE-TX/10BASE-T register set.
	 */
	...
	if (bmcr & BMCR_ANENABLE) {
		...
		return (anlpar & (LPA_10FULL | LPA_100FULL)) ? FULL : HALF;
	}
	/* Get speed from basic control settings. */
